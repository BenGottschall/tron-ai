...
class RLAgent:
    ...
    def get_state(self, game_board, player, opponent):
        ...
        wall_distances = self.get_wall_distances(game_board, player)
        state = np.concatenate([state.flatten(), np.array(wall_distances)], axis=0)
        state = np.concatenate([state, np.array([player.x, player.y] + player.direction)], axis=0)
