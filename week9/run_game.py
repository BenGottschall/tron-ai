...
def run_game(ai_class1, ai_class2, model_file1=None, model_file2=None):
    ...
    ai1 = RLAgent(action_size=4, player_id=1, grid_size=21, model_file=model_file1) if ai_class1 == RLAgent else ai_class1()
    ai2 = RLAgent(action_size=4, player_id=2, grid_size=21, model_file=model_file2) if ai_class2 == RLAgent else ai_class2()
    
    # Validate models after loading
    dummy_input = torch.zeros((1, 3, 21, 21)).to(ai1.device)
    with torch.no_grad():
...
if __name__ == "__main__":
    run_game(RLAgent, RLAgent, "tron_model_player1_high_score.pth", "tron_model_player2_high_score.pth")